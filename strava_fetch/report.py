...
def _agg_runs(runs: pd.DataFrame) -> pd.DataFrame:
    ...
    return runs.groupby("week_start").agg(
...
def _agg_rides(rides: pd.DataFrame) -> pd.DataFrame:
    ...
    return rides.groupby("week_start").agg(
...
def build_weekly_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not runs.empty:
        ...
        for col in ("num_run_activities", "total_run_distance_km", "run_moving_s",
                    "avg_run_hr_bpm", "longest_run_km"):
            spine[col] = float("nan")
    ...
    if not rides.empty:
        ...
        for col in ("num_cycling_activities", "total_cycling_distance_km",
                    "cycling_moving_s", "avg_cycling_power_w", "avg_cycling_hr_bpm"):
            spine[col] = float("nan")
    ...
    result = spine[[
        ...
        "num_cycling_activities", "total_cycling_distance_km",
        "total_cycling_time", "avg_cycling_power_w", "avg_cycling_hr_bpm",
    ]].sort_values("week_start", ascending=False).reset_index(drop=True)
